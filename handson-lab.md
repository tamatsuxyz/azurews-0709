...
~~Contoso Inc. のニーズに合わせて、Azure Storage の高度な設定を行います。データのバックアップとライフサイクル管理を設定し、ストレージの運用効率を高めます。~~
...
4. バックアップ ポリシーを新規に作成します。バックアップ ポリシーのパラメータは以下を入力します。
   [基本] タブ
   ・ポリシー名：BlobBackup
